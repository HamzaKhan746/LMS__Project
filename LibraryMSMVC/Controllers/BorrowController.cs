...
namespace LibraryMSMVC.Controllers
{
    public class BorrowController : Controller
    {
        ...
        public ActionResult Index(int? userId, string userName)
        {
            ...
            return View(context.tblBooks.ToList());
        }
        ...
        public ActionResult Borrow(int? bookId)
        {
            /*try
            {*/
                if (context.tblTransactions.Where(t => t.UserId == userId).Count() < 6)
                {
                    if (bookId != null)
                    {
                        tblBook book = context.tblBooks.FirstOrDefault(b => b.BookId == bookId);
                        if (book == null)
                        {
                            return HttpNotFound();
                        }
                        if (book.BookCopies > 0)
                        {
                            ...
                            context.SaveChanges();
                            context.tblTransactions.Add(trans);
                            context.SaveChanges();
